...
const savingTimes = computed(() => { return store.getBenefit.savingTimes })
...
onMounted(async () => {
  store.setSavingTimes(savingTimes.value)
...
const setValidateString = async () => {
  ...
  if (benefitType.value === 'Efficiency') store.setSpecificValidate('benefit.savingTimes', validateString)
}
